...
import { fetchAllGithubReleases, GitHubRelease } from './github';
import {
  type ChannelName,
  parseUpdateConfig,
  type UpdateConfig,
...
export class generateManifest {
  async generateManifest() {
    const releases = await fetchAllGithubReleases(
      ...
      .then(asyncMap(this.parseRelease))
      .then((results) =>
        results.filter(
          (
            r
    ...
    fs.writeFileSync(
      `manifests/${UPDATE_MANIFEST_FILE}`,
  ...
  async parseRelease(
    ...
    | [ChannelName, SemVer, Version, UpdateManifestChannelVersion, UpdateConfig]
    | null
  > {
    ...
    const releasedUpdateConfig = release.assets.find(
      (asset) => asset.name === UPDATE_CONFIG_FILENAME
    );
    ...
    const builds: Record<
      string,
      Record<string, UpdateManifestChannelVersionBuild>
    > = {};

    const windowsx8664ZIP = release.assets.find(
      (asset) => asset.name === WINDOWS_X86_64_ZIP_FILENAME
    );
    if (windowsx8664ZIP) {
      builds.windows = {
        x86_64: {
    ...
    const linuxAMD64AppImage = release.assets.find(
      (asset) =>
        asset.name.toLowerCase() === LINUX_AMD64_APPIMAGE_FILENAME.toLowerCase()
    );
    if (linuxAMD64AppImage) {
      builds.linux = {
        x86_64: {
    ...
    for (const [platform, architectures] of Object.entries(builds)) {
      for (const [arch, build] of Object.entries(architectures)) {
        builds[platform][arch] = {
          ...
          run: build.run.map((arg) =>
            arg.replaceAll('$VERSION$', version.toString())
          ),
        };
      }
    }
