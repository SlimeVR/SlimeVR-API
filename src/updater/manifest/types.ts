...
export const UpdateManifestChannelVersionBuildSchema = v.object({
  ...
  checksum: v.pipe(v.string(), v.string()),
  run: v.pipe(v.array(v.string())),
});
export type UpdateManifestChannelVersionBuild = v.InferOutput<
  typeof UpdateManifestChannelVersionBuildSchema
>;
...
export const UpdateManifestChannelVersionSchema = v.object({
  ...
  builds: v.pipe(
    v.record(
      v.string(),
      v.record(v.string(), UpdateManifestChannelVersionBuildSchema)
    )
  ),
});
export type UpdateManifestChannelVersion = v.InferOutput<
  typeof UpdateManifestChannelVersionSchema
>;
...
export type UpdateManifestChannel = v.InferOutput<
  typeof UpdateManifestChannelSchema
>;
...
export const parseUpdateManifest = (
  data: string
): v.InferOutput<typeof UpdateManifestSchema> =>
...
export const parseUpdateConfig = (
  data: string
): v.InferOutput<typeof UpdateConfigSchema> =>
